...
To start you just need a `ggplot` and some text you wish to encode into the QR. The QR is intended to contain enough information to uniquely identify the report, so a URL, file name or other unique identifier. The QR encode process automatically appends a time stamp and the current git `sha` code if available. But try to keep the content of the QR code minimal. The more information encoded, the more pixels are required and the larger the QR needs to be. The examples here need a QR code size of 1.8cm to be reliably scanned using a phone off the computer monitor. Printed codes can scan reliably when much smaller. QR codes are encoded using the [qrencoder](https://github.com/hrbrmstr/qrencoder) package.
